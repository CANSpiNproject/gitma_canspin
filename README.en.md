...
*Read this file in different languages: [German](README.md)*
...
- Clone the repository into a project folder: `git clone https://cls-gitlab.phil.uni-wuerzburg.de/canspin/gitma-canspin.git`.
...
## Getting started
In addition to the following step-by-step instructions and explanations, you will find ready-to-use scripts and Jupyter Notebooks in the `CANSpiN-scripts` repo in the `Templates/gitma_canspin` folder. These scripts and Notebooks are intended to be copied into the project folder and executed there. In order to work, the necessary settings must first be made in the templates (including the definition of a user-bound personal access token for the CATMA backend and filenames for import and export).

Work in progress: Currently, a **Streamlit** app is being added to the package that allows all the features of the *canspin* module to be used in a GUI. Once the virtual environment is activated, the app can be started from every folder in the terminal: `gui-start`.

Independently of the reference to the prepared files in the `CANSpiN-scripts` repo and the reference to the Streamlit app, step-by-step instructions for creating your own Jupyter Notebooks and scripts now follow. Three classes are currently provided for working with the package: [AnnotationExporter](#annotationexporter), [AnnotationAnalyzer](#annotationanalyzer) and [AnnotationManipulator](#annotationmanipulator). All three depend on a Catma project being loaded first. This is demonstrated below using the loading of an exporter.

- Create a new Python script or a Jupyter Notebook in the project folder. The folder structure should look like this:
    ```
    <project folder>
      <gitma-canspin>
      my_script.py
...
- In the new Python file: Import the `AnnotationExporter` class from *canspin* module of *gitma_canspin*:
    ...
    ```
- Define the initialization settings for the exporter, here for the Catma project with the German CANSpiN texts:
    ```python
    ...
    ```
  - `project_name` (str, default: `'CATMA_5D2A90F0-4428-41CB-9D3A-E649CD1702C2_CANSpiN'`) is defined in Catma and corresponds to the folder name of the project, which can be obtained by using a respective function placed at the beginning of the jupyter notebook templates in the `CANSpiN-scripts` repo or by logging into the Catma backend ([https://git.catma.de](https://git.catma.de)). The default value is the folder name of the Catma project for the German texts in the CANSpiN project.
...
- Create an `AnnotationExporter` instance with the initialization settings we just defined:
    ...
    ```
    If the exporter has received the correct init values and the project download was successful, a list of all loaded annotation collections is displayed in the terminal and the folder structure now looks like this:
    ```
    <project folder>
...
### AnnotationExporter
This class is designed to export CATMA annotations into `.tsv` and TEI-conform `.xml` files. This step is part of the project pipeline: preparing the annotations for usage in training classifiers using NTEE.
